...
from app.schemas.livestream.web_rtc import WebRTCSessionResponseSchema, WebRTCSessionCreateSchema, \
...
async def create_webrtc_session(stream_id: PydanticObjectId, payload: WebRTCSessionCreateSchema):
    session = await LiveStreamService.create_webrtc_session(
        stream_id=stream_id,
...
async def get_sessions(stream_id: PydanticObjectId):
    sessions = await LiveStreamService.get_sessions(stream_id)
    return sessions
...
async def add_webrtc_peer(session_id: PydanticObjectId, payload: WebRTCPeerCreateSchema):
    peer = await LiveStreamService.add_webrtc_peer(
...
async def get_peers(session_id: PydanticObjectId):
    peers = await LiveStreamService.get_peers(session_id)
    return peers
...
async def remove_peer(session_id: PydanticObjectId, peer_id: str):
    await LiveStreamService.remove_webrtc_peer(session_id, peer_id)
